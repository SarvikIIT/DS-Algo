...
void dfs(int root, int parent) {
    vis[root] = true;
    if (a[root] == 1) { // Cat Counter
        BadScore[root] = 1 + BadScore[parent];
    }
    else BadScore[root] = 0;
    if (BadScore[root] > m)return;
    if (ar[root].size() == 1 && ar[root][0] == parent)score++; //If i reach a leaf node that follows my rule then score update
